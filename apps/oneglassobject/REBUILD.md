...
**画廊图录（catalogue），坚持全白，不用深色区块。**

全白配色下没有明暗对比可用，层次只能来自四件事——后续所有页面都按这个来：

1. **可见版心**：12 栏 `.grid12`，元素落在不同起止列上形成非对称，不要居中对齐
2. **发丝线**：`.index-rule` 章节页眉（编号 + 标签 + 右侧注记），像图录页眉
3. **字号级差**：标题拉到极大（`.type-xl`），标签压到极小全大写，中间档位一律不用
4. **错位与留白**：图文不对齐、故意留大块空白

具体禁忌：
- ❌ 不要"居中小标题 + 居中大标题 + 居中按钮 + 等大卡片网格"——那是通用电商模板节奏
- ❌ 商品不要排成方阵，用左右交替的编号索引
- ✅ 参数用 `.spec` 博物馆展签式（`.spec-key` / `.spec-val`）
- ✅ 每个条目带 `.num` 编号

可复用的图录组件（都在 `globals.css` 的 CATALOGUE 段）：
`.grid12` `.num` `.index-rule` `.spec` `.type-xl` `.frame-empty` `.frame-sheen`
...
业主看过第一版首页后反馈"整体还是那个布局"。诊断正确且重要：
第一版只换了皮肤（颜色、字体、文案），没换构图语言，骨架仍是
"居中标题 + 卡片网格"的通用电商模板节奏，所以读起来跟丝带站一样。

已按画廊图录重排首页：
- Hero 非对称——标题落 1–8 栏、第二行缩进，图片落 9–13 栏并上提，副文案落 3–7 栏
- 精选作品改成左右交替的编号索引（之字形），每件配 `.spec` 展签
- 系列改成竖排编号目录，发丝线分行，悬停行内缩进 + 缩略图淡入
- 每个章节带 `.index-rule` 编号页眉（01–05）
