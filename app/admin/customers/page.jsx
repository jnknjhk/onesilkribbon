    ...
    <div style={{ display: 'flex', gap: 24, height: 'calc(100vh - 80px)' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ color: '#1C1714', fontSize: 24, fontWeight: 300, marginBottom: 20 }}>客户管理</h1>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索客户邮箱或姓名…"
            style={{ width: '100%', padding: '9px 14px', background: '#FFFFFF', border: '1px solid #E8E4DF', borderRadius: 8, color: '#1C1714', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', background: '#FFFFFF', border: '1px solid #E8E4DF', borderRadius: 12 }}>
          {loading ? <p style={{ color: '#A8A4A0', padding: 24 }}>加载中…</p> : filtered.length === 0 ? (
            <p style={{ color: '#A8A4A0', padding: 24, fontSize: 13 }}>暂无客户数据</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filtered.map(c => (
                  <tr key={c.email} onClick={() => setSelected(c)} style={{ borderBottom: '1px solid #F0EDE8', cursor: 'pointer', background: selected?.email === c.email ? '#FBF8F4' : 'transparent' }}>
                    <td style={{ padding: '12px 16px' }}>
                      <p style={{ color: '#2E2B28', fontSize: 13 }}>{c.name || '-'}</p>
                      <p style={{ color: '#A8A4A0', fontSize: 11 }}>{c.email}</p>
                    </td>
                    <td style={{ padding: '12px 16px', color: '#8A8480', fontSize: 12 }}>{[c.city, c.country].filter(Boolean).join(', ') || '-'}</td>
                    <td style={{ padding: '12px 16px', color: '#504C48', fontSize: 13 }}>{c.orders}</td>
                    <td style={{ padding: '12px 16px', color: '#B89B6A', fontSize: 13 }}>{fmt(c.spent)}</td>
                    <td style={{ padding: '12px 16px', color: '#A8A4A0', fontSize: 12 }}>{c.lastOrder ? new Date(c.lastOrder).toLocaleDateString('zh-CN') : '-'}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <a href={`mailto:${c.email}`} onClick={e => e.stopPropagation()}
                        style={{ background: '#EDE9E4', border: 'none', borderRadius: 4, color: '#B89B6A', fontSize: 11, padding: '5px 12px', cursor: 'pointer', textDecoration: 'none', display: 'inline-block' }}>
                        发邮件
                      </a>
                    </td>
                  </tr>
      ...
      {selected && (
        <div style={{ width: 280, background: '#FFFFFF', border: '1px solid #E8E4DF', borderRadius: 12, padding: 24, flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
            <h2 style={{ color: '#1C1714', fontSize: 15, fontWeight: 400 }}>客户详情</h2>
            <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', color: '#A8A4A0', cursor: 'pointer', fontSize: 18 }}>×</button>
          </div>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#EDE9E4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, marginBottom: 20 }}>
          ...
          <a href={`mailto:${selected.email}`}
            style={{ display: 'block', textAlign: 'center', padding: '10px', background: '#B89B6A', borderRadius: 6, color: '#1C1714', fontSize: 12, letterSpacing: '.15em', textDecoration: 'none', marginTop: 20 }}>
            发送邮件
          </a>
        </div>
      )}
