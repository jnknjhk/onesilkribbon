    ...
    <div style={{ display: 'flex', gap: 24, height: 'calc(100vh - 80px)' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ color: '#fff', fontSize: 24, fontWeight: 300, marginBottom: 20 }}>订单管理</h1>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索邮箱或订单号…"
            style={{ width: '100%', padding: '9px 14px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 8, color: '#fff', fontSize: 13, outline: 'none', marginBottom: 12, boxSizing: 'border-box' }} />
        ...
        <div style={{ flex: 1, overflow: 'auto', background: '#1a1a1a', border: '1px solid #222', borderRadius: 12 }}>
          {loading ? <p style={{ color: '#555', padding: 24 }}>加载中…</p> : filtered.length === 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filtered.map(o => (
                  <tr key={o.id} onClick={() => setSelected(o)} style={{ borderBottom: '1px solid #1f1f1f', cursor: 'pointer', background: selected?.id === o.id ? '#1f1f1f' : 'transparent' }}>
                    <td style={{ padding: '12px 16px', color: '#888', fontSize: 11, fontFamily: 'monospace' }}>{(o.id || '').slice(0,8)}…</td>
                    <td style={{ padding: '12px 16px', color: '#ccc', fontSize: 12 }}>{o.customer_email || '-'}</td>
                    ...
                    <td style={{ padding: '12px 16px', color: '#555', fontSize: 11 }}>{o.created_at ? new Date(o.created_at).toLocaleDateString('zh-CN') : '-'}</td>
                    <td style={{ padding: '12px 16px' }} onClick={e => e.stopPropagation()}>
                      <select value={o.status || ''} onChange={e => updateStatus(o.id, e.target.value)}
                        style={{ background: '#111', border: '1px solid #333', borderRadius: 6, color: '#ccc', fontSize: 11, padding: '4px 8px', cursor: 'pointer' }}>
      ...
      {selected && (
        <div style={{ width: 320, background: '#1a1a1a', border: '1px solid #222', borderRadius: 12, padding: 24, overflow: 'auto', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
            <h2 style={{ color: '#fff', fontSize: 15, fontWeight: 400 }}>订单详情</h2>
          ...
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid #222' }}>
            <p style={{ color: '#444', fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 10 }}>更新状态</p>
