...
export async function generateMetadata({ params }) {
  ...
  // 根布局的 title.template 会自动在 <title> 后面拼上 "| One Glass Object"，
  // 这里给 <title> 用的是不带品牌后缀的短标题；openGraph/twitter 不走 template，
  // 单独给一个带完整品牌的版本，社交平台分享卡片上才不会显得没头没尾。
  const title = product.name
  const socialTitle = `${product.name} — One Glass Object`
  const description = product.description
    ? product.description.replace(/<[^>]+>/g, '').slice(0, 160)
    : `Handmade 100% mulberry silk ribbon, hand-dyed in the UK. Perfect for weddings and bouquets. Shop ${product.name} at One Glass Object.`
  ...
  return {
    ...
    openGraph: {
      title: socialTitle,
      description,
      url: `https://oneglassobject.com${canonicalPath}`,
...
export default async function ProductPage({ params }) {
  ...
  const productUrl = `https://oneglassobject.com/collections/${slug}/${productSlug}`
  ...
  const jsonLd = product ? {
    ...
    brand: { '@type': 'Brand', name: 'One Glass Object' },
    ...
    ...(hasValidOffer ? {
      offers: {
        ...
        url: productUrl,
        seller: { '@type': 'Organization', name: 'One Glass Object', url: 'https://oneglassobject.com' },
      },
    } : {}),
    breadcrumb: {
      '@type': 'BreadcrumbList',
      itemListElement: [
        { '@type': 'ListItem', position: 1, name: 'Collections', item: 'https://oneglassobject.com/collections' },
        { '@type': 'ListItem', position: 2, name: product.collection?.replace(/-/g, ' ') || 'Products', item: `https://oneglassobject.com/collections/${product.collection}` },
