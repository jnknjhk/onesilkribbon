...
const FROM       = `One Glass Object <${process.env.EMAIL_FROM || 'hello@oneglassobject.com'}>`
const OWNER      = 'hello@oneglassobject.com'
...
<div class="wrapper">
  <div class="header">
    <h1>One Glass Object</h1>
    <p>Handcrafted Mulberry Silk</p>
  </div>
  <div class="body">${content}</div>
  <div class="footer">
    <p>
      <a href="https://oneglassobject.com">oneglassobject.com</a> &nbsp;·&nbsp;
      <a href="mailto:hello@oneglassobject.com">hello@oneglassobject.com</a>
    </p>
    <p style="margin-top:8px;">© ${new Date().getFullYear()} One Glass Object. All rights reserved.</p>
    <p style="margin-top:8px;">
      <a href="https://oneglassobject.com/privacy">Privacy Policy</a> &nbsp;·&nbsp;
      <a href="https://oneglassobject.com/terms">Terms of Service</a>
    </p>
  </div>
</div>
</body>
...
export async function sendOrderConfirmation({ order, items, form, totals }) {
  ...
  const html = layout(`
    ...
    <div class="order-box">
      ...
      ${itemsHtml}
      <div class="totals">
        <div class="total-row"><span>Subtotal (tax included)</span><span>£${totals.subtotal || totals.subtotalExVat || '0.00'}</span></div>
    ...
    <a href="https://oneglassobject.com/track-order?order=${order.order_number}" class="btn">Track Your Order</a>

    <hr class="divider">
    <p class="text" style="font-size:13px;">
      Questions about your order? Reply to this email or contact us at 
      <a href="mailto:hello@oneglassobject.com" class="gold">hello@oneglassobject.com</a>
    </p>
  `)

  await sendEmail({
    to: form.email,
    subject: `Order Confirmed — ${order.order_number} | One Glass Object`,
    html,
  })
}
...
export async function sendOwnerNotification({ order, items, form, totals }) {
  const itemsText = items.map(i => `${i.name}${i.skuDesc ? ' · ' + i.skuDesc : ''} × ${i.qty} · £${Number(i.price).toFixed(2)} each — £${(i.price * i.qty).toFixed(2)}`).join('<br>')
  ...
  const html = layout(`
    ...
    <a href="https://oneglassobject.com/admin/orders" class="btn">View in Admin</a>
  `)
...
export async function sendShippingNotification({ order, trackingNumber, carrier, trackingUrl }) {
  const html = layout(`
    ...
    <p class="text">
      ...
      Your One Glass Object order has been carefully packaged and handed to our courier. 
    ...
    <hr class="divider">
    <p class="text" style="font-size:13px;">
      Orders are shipped by air and typically arrive within 5–14 days of dispatch.
  ...
  await sendEmail({
    ...
    subject: `Your order is on its way — ${order.order_number} | One Glass Object`,
    html,
  })
}
