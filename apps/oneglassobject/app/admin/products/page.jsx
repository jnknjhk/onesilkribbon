'use client'
import { COLLECTIONS as SITE_COLLECTIONS } from '@/config/site'
...
const COLLECTIONS = SITE_COLLECTIONS.map(c => ({ value: c.slug, label: c.adminLabel }))
const DEFAULT_COLLECTION = SITE_COLLECTIONS[0].slug
...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({ name: '', slug: '', description: '', collection: DEFAULT_COLLECTION, active: true })
  ...
  async function startEdit(product) {
    if (product === 'new') {
      setForm({ name: '', slug: '', description: '', collection: DEFAULT_COLLECTION, active: true })
      ...
      setForm({
        ...
        collection: product.collection || DEFAULT_COLLECTION,
  ...
  return (
    <div>
      ...
      <div className="admin-products-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button onClick={() => startEdit('new')} className="admin-tap-target" style={{ background: C.gold, border: 'none', borderRadius: 8, color: '#fff', fontSize: 12, padding: '10px 24px', cursor: 'pointer' }}>+ 新建产品</button>
      </div>
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: '1px solid #F0EDE8', cursor: 'pointer' }} onClick={() => startEdit(p)}>
                    ...
                    <td data-label="操作" style={{ padding: '10px 16px' }} onClick={e => e.stopPropagation()}>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => startEdit(p)} className="admin-tap-target" style={{ background: C.light, border: 'none', borderRadius: 4, color: C.gold, fontSize: 11, padding: '5px 12px', cursor: 'pointer' }}>编辑</button>
                        <button onClick={() => requestDuplicate(p)} className="admin-tap-target" style={{ background: C.light, border: 'none', borderRadius: 4, color: C.sub, fontSize: 11, padding: '5px 12px', cursor: 'pointer' }}>复制</button>
                        <button onClick={() => requestDelete(p)} className="admin-tap-target" style={{ background: C.light, border: 'none', borderRadius: 4, color: C.red, fontSize: 11, padding: '5px 12px', cursor: 'pointer' }}>删除</button>
                      </div>
                    </td>
                  </tr>
                )
