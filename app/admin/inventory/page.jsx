  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
        <div>
          <h1 style={{ color: '#1C1714', fontSize: 24, fontWeight: 300, marginBottom: 8 }}>库存管理</h1>
          {lowStock > 0 && <p style={{ color: '#facc15', fontSize: 12 }}>⚠ {lowStock} 个SKU库存低于10</p>}
        </div>
        <p style={{ color: '#A8A4A0', fontSize: 13 }}>共 {skus.length} 个SKU</p>
      </div>

      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索颜色或产品名…"
          style={{ flex: 1, padding: '9px 14px', background: '#FFFFFF', border: '1px solid #E8E4DF', borderRadius: 8, color: '#1C1714', fontSize: 13, outline: 'none' }} />
        <select value={filterProduct} onChange={e => setFilterProduct(e.target.value)}
          style={{ padding: '9px 14px', background: '#FFFFFF', border: '1px solid #E8E4DF', borderRadius: 8, color: '#504C48', fontSize: 13, outline: 'none' }}>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E8E4DF', borderRadius: 12, overflow: 'hidden' }}>
        {loading ? <p style={{ color: '#A8A4A0', padding: 24 }}>加载中…</p> : (
              ...
              {filtered.map(s => {
                ...
                return (
                  <tr key={s.id} style={{ borderBottom: '1px solid #F0EDE8' }}>
                    <td style={{ padding: '11px 16px', color: '#6B6460', fontSize: 12 }}>{productName(s.product_id)}</td>
                    <td style={{ padding: '11px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {s.colour_hex && <div style={{ width: 14, height: 14, borderRadius: '50%', background: s.colour_hex, border: '1px solid #DDD8D2', flexShrink: 0 }} />}
                        <span style={{ color: '#504C48', fontSize: 12 }}>{s.colour || '-'}</span>
                      </div>
                    </td>
                    <td style={{ padding: '11px 16px', color: '#8A8480', fontSize: 12 }}>{s.length_metres ? `${s.length_metres}m` : s.width_mm ? `${s.width_mm}mm` : '-'}</td>
                    <td style={{ padding: '11px 16px', color: '#B89B6A', fontSize: 12 }}>{s.price_gbp ? `£${s.price_gbp}` : '-'}</td>
                    <td style={{ padding: '11px 16px' }}>
                      {editingId === s.id ? (
                        <input type="number" value={editVal} onChange={e => setEditVal(e.target.value)}
                          autoFocus style={{ width: 70, padding: '4px 8px', background: '#F5F3F0', border: '1px solid #B89B6A', borderRadius: 4, color: '#1C1714', fontSize: 12, outline: 'none' }} />
                      ) : (
                      ...
                      {editingId === s.id ? (
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => saveStock(s.id)} disabled={saving}
                            style={{ background: '#B89B6A', border: 'none', borderRadius: 4, color: '#1C1714', fontSize: 11, padding: '4px 10px', cursor: 'pointer' }}>
                            保存
                          </button>
                          <button onClick={() => setEditingId(null)}
                            style={{ background: '#EDE9E4', border: 'none', borderRadius: 4, color: '#8A8480', fontSize: 11, padding: '4px 10px', cursor: 'pointer' }}>
                            取消
                          </button>
                        </div>
                      ) : (
