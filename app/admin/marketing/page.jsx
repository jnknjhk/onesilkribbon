  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <div>
          <h1 style={{ color: '#fff', fontSize: 24, fontWeight: 300, marginBottom: 8 }}>营销管理</h1>
          <p style={{ color: '#555', fontSize: 13 }}>创建和管理优惠码</p>
        </div>
        <button onClick={() => { setForm(EMPTY); setEditing(null); setShowForm(true) }}
          style={{ background: '#B89B6A', border: 'none', borderRadius: 8, color: '#fff', fontSize: 12, padding: '10px 20px', cursor: 'pointer', letterSpacing: '.1em' }}>
          + 新建优惠码
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>
        {[
          ...
          <div key={label} style={{ background: '#1a1a1a', border: '1px solid #222', borderRadius: 12, padding: '20px 24px' }}>
            <p style={{ color: '#555', fontSize: 11, letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 10 }}>{label}</p>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
        {/* Table */}
        <div style={{ flex: 1, background: '#1a1a1a', border: '1px solid #222', borderRadius: 12, overflow: 'hidden' }}>
          {loading ? <p style={{ color: '#555', padding: 24 }}>加载中…</p> : coupons.length === 0 ? (
            <p style={{ color: '#555', padding: 24, fontSize: 13 }}>暂无优惠码，点击右上角新建</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {coupons.map(c => {
                  ...
                  return (
                    <tr key={c.id} style={{ borderBottom: '1px solid #1f1f1f', opacity: expired ? 0.5 : 1 }}>
                      <td style={{ padding: '13px 16px' }}>
                        <p style={{ color: '#fff', fontSize: 13, fontFamily: 'monospace', letterSpacing: '.08em' }}>{c.code}</p>
                        {c.description && <p style={{ color: '#555', fontSize: 11, marginTop: 3 }}>{c.description}</p>}
                      </td>
                      <td style={{ padding: '13px 16px', color: '#B89B6A', fontSize: 13 }}>{fmtDiscount(c)}</td>
                      <td style={{ padding: '13px 16px', color: '#888', fontSize: 12 }}>{c.min_order_gbp > 0 ? `£${c.min_order_gbp}` : '无要求'}</td>
                      <td style={{ padding: '13px 16px', color: '#888', fontSize: 12 }}>
                      ...
                      <td style={{ padding: '13px 16px' }}>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => startEdit(c)} style={{ background: '#2a2a2a', border: 'none', borderRadius: 4, color: '#B89B6A', fontSize: 11, padding: '5px 10px', cursor: 'pointer' }}>编辑</button>
                          <button onClick={() => deleteCoupon(c.id)} style={{ background: '#2a2a2a', border: 'none', borderRadius: 4, color: '#f87171', fontSize: 11, padding: '5px 10px', cursor: 'pointer' }}>删除</button>
                        </div>
                      </td>
                    </tr>
                  )
        ...
        {/* Form panel */}
        {showForm && (
          <div style={{ width: 340, background: '#1a1a1a', border: '1px solid #222', borderRadius: 12, padding: 24, flexShrink: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ color: '#fff', fontSize: 15, fontWeight: 400 }}>{editing ? '编辑优惠码' : '新建优惠码'}</h2>
              <button onClick={() => { setShowForm(false); setEditing(null) }} style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer', fontSize: 18 }}>×</button>
            </div>
            ...
            {[
              ...
              <div key={key} style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', color: '#444', fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 6 }}>{label}</label>
                <input type={type} value={form[key]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} placeholder={placeholder}
                  style={{ width: '100%', padding: '9px 12px', background: '#111', border: '1px solid #2a2a2a', borderRadius: 6, color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            ))}

            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', color: '#444', fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 6 }}>折扣类型</label>
              <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                ...
                <span style={{ color: '#ccc', fontSize: 13 }}>立即生效</span>
              </label>
            </div>

            {msg && <p style={{ color: msg.includes('✓') ? '#4ade80' : '#f87171', fontSize: 12, marginBottom: 12 }}>{msg}</p>}
            <button onClick={saveCoupon} disabled={saving}
              style={{ width: '100%', padding: '11px', background: '#B89B6A', border: 'none', borderRadius: 6, color: '#fff', fontSize: 12, letterSpacing: '.15em', cursor: 'pointer' }}>
