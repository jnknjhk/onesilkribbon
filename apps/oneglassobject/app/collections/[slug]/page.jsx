...
export async function generateMetadata({ params }) {
  ...
  // 根布局的 title.template 会自动拼上 "| One Glass Object"，<title> 用短标题；
  // openGraph/twitter 不走 template，单独给带完整品牌的版本
  const title = meta.name
  const socialTitle = `${meta.name} | One Glass Object`
  ...
  return {
    ...
    openGraph: {
      title: socialTitle,
      description: meta.desc,
      url: `https://oneglassobject.com/collections/${slug}`,
  ...
  const jsonLd = {
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: meta?.name || slug, item: `https://oneglassobject.com/collections/${slug}` },
      ],
    },
  }
